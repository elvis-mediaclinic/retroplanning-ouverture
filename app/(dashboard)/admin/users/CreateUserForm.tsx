      ...
      {state?.inviteLink ? (
        <div className="space-y-3">
          <p className="text-sm text-green-700">
            ✓ Compte créé pour <strong>{state.nom}</strong> ({state.email}).
          </p>
          <div className="space-y-1">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 rounded-md border border-zinc-300 px-3 py-2 text-xs text-zinc-500 bg-zinc-50 font-mono truncate"
              />
              <button
                type="button"
                onClick={handleCopy}
                className="shrink-0 rounded-md border border-zinc-300 px-3 py-2 text-xs text-zinc-700 hover:bg-zinc-50"
              >
                {copied ? "✓ Copié" : "Copier"}
              </button>
            </div>
            <p className="text-xs text-zinc-400">
              Ce lien expire après utilisation. La personne choisira son mot de passe en cliquant dessus.
            </p>
          </div>
        </div>
      ) : (
